...
public class GuestbookDAO {
	...
	public boolean updateOne(GuestbookVO book) throws SQLException {
		...
		//connection 연결확인 하세요
		Connection con = ConnectionManagerV2.getConnection();
//		String sql = "update guestbook set title = ?, content=?, read_count=? where seq = ?"; // 업데이트에 필요 없는 부분 제거
		String sql = "update guestbook set title = ?, content=? where seq = ?";
		PreparedStatement pstmt = con.prepareStatement(sql);
		pstmt.setString(1, book.getTitle());
		//System.out.println("dao "+book.getContent());
		pstmt.setString(2, book.getContent());
//		pstmt.setInt(3, book.getReadCount()); // 업데이트에 필요없는 부분 제거
		pstmt.setInt(3, book.getSeq());
